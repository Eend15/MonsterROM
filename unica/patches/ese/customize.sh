...
{
    if ! SMALI_PATCH "$@"; then
        LOGW "eSE smali layout differs on this firmware; retaining the stock implementation"
    fi
    return 0
}
# ]
...
if [[ "$SOURCE_SECURITY_CONFIG_ESE_CHIP_VENDOR" == "NXP" ]] && [[ "$SOURCE_SECURITY_CONFIG_ESE_COS_NAME" == "JCOP6.2U" ]] && \
    ...
    if [[ "$SOURCE_SECURITY_CONFIG_ESE_COS_NAME" != "$TARGET_SECURITY_CONFIG_ESE_COS_NAME" ]]; then
        SMALI_OPTIONAL_PATCH "system" "system/app/SecureElement/SecureElement.apk" \
    ...
    if [[ "$SOURCE_SECURITY_CONFIG_ESE_CHIP_VENDOR" != "$TARGET_SECURITY_CONFIG_ESE_CHIP_VENDOR" ]]; then
        SMALI_OPTIONAL_PATCH "system" "system/framework/services.jar" \
...
unset -f LOG_MISSING_PATCHES SMALI_OPTIONAL_PATCH
